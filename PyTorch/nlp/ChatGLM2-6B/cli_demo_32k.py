...
def build_prompt(history):
    prompt = "欢迎使用 ChatGLM2-6B-32K 模型，输入内容即可进行对话，clear 清空对话历史，stop 终止程序"
    for query, response in history:
        prompt += f"\n\n用户：{query}"
        prompt += f"\n\nChatGLM2-6B-32K：{response}"
    return prompt
...
def main():
    ...
    global stop_stream
    print("欢迎使用 ChatGLM2-6B-32K 模型，输入内容即可进行对话，clear 清空对话历史，stop 终止程序")
    while True:
        query = input("\n用户：")
        if query.strip() == "stop":
            break
